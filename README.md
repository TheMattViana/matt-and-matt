...
| 🏆 **Garage Draft** | An NBA-roulette-style car draft. Each turn the game **rolls a random era + class** (e.g. "Modern × Utility") and you draft one car from a healthy pool into an open **position** — Speed, Launch, Corners, Hauler or Trail. Classes are broad (Performance / Utility), so every roll always gives you several cars to choose from; if a class runs thin, the roll widens to a **Wildcard** of the whole era. You get **one retool** (re-roll) for the whole game. Fill all five positions and the squads battle **head-to-head, position by position** — your Speed car vs theirs in the Top Speed Shootout, your Hauler vs theirs in the Tow-Off, etc. Best of five wins. Stats stay hidden while you draft, so you're picking on car knowledge — and a roll might force a supercar into your Hauler slot. |
| 🎨 **Doodle Drift** | A two-phone game of telephone with drawings — **no winner, just laughs**. The game hands out a **secret prompt** and you draw it. Your partner sees *only your drawing* (never the words), **guesses** what it is, then **draws their own guess**. It bounces back and forth for three drawings, each one drifting further from the last. At the end the **whole chain is revealed** — prompt → scribble → wrong guess → scribble → … — so you can see how "a snail with a rocket" became "a very confused octopus". Both of you draw *and* guess. Every game rolls a fresh prompt from a big pile. |
| 🐧 **Icebreaker** | Each turn is two actions: **move one square** (moving breaks nothing), then **break any one block** on the map. Breaking is your weapon — strand your rival so the ice gives way. You can't break the block a penguin stands on, and **no floating blocks**: when every square around a penguin is broken (or the edge), it falls through. The floe crumbles as you play, so a game always ends with a winner. |
...
Grand Prix and Flappy Duel are **solo-run duels** — you each take your shot on the same course and compare scores. Doodle Drift is turn-by-turn but has **no winner** — it's the one game you play just to laugh at the result. The rest, including Garage Draft, are **turn-by-turn** (Garage Draft alternates rolls & picks, then simulates the head-to-head from the finished squads — all deterministic from the shared link, so both players see the same rolls and results).
